'use strict';

module.exports = {
  up: (queryInterface, Sequelize) => {
    return queryInterface.bulkInsert('ActivityLogs', [
      {
        point: 3,
        hero_id: 2,
        createdAt: new Date(2020, 9, 1),
        updatedAt: new Date()
      },
      {
        point: 2,
        hero_id: 2,
        createdAt: new Date(2020, 9, 2),
        updatedAt: new Date()
      },
      {
        point: 8,
        hero_id: 2,
        createdAt: new Date(2020, 9, 3),
        updatedAt: new Date()
      },
      {
        point: 6,
        hero_id: 2,
        createdAt: new Date(2020, 9, 4),
        updatedAt: new Date()
      },
      {
        point: 1,
        hero_id: 2,
        createdAt: new Date(2020, 9, 5),
        updatedAt: new Date()
      },
      {
        point: 6,
        hero_id: 2,
        createdAt: new Date(2020, 9, 6),
        updatedAt: new Date()
      },
      {
        point: 2,
        hero_id: 2,
        createdAt: new Date(2020, 9, 7),
        updatedAt: new Date()
      },
      {
        point: 6,
        hero_id: 2,
        createdAt: new Date(2020, 9, 8),
        updatedAt: new Date()
      },
      {
        point: 1,
        hero_id: 2,
        createdAt: new Date(2020, 9, 9),
        updatedAt: new Date()
      },
      {
        point: 9,
        hero_id: 2,
        createdAt: new Date(2020, 9, 9),
        updatedAt: new Date()
      },
      {
        point: 6,
        hero_id: 2,
        createdAt: new Date(2020, 9, 11),
        updatedAt: new Date()
      },
      {
        point: 2,
        hero_id: 2,
        createdAt: new Date(2020, 9, 12),
        updatedAt: new Date()
      },
      {
        point: 3,
        hero_id: 2,
        createdAt: new Date(2020, 9, 13),
        updatedAt: new Date()
      },
      {
        point: 8,
        hero_id: 2,
        createdAt: new Date(2020, 9, 14),
        updatedAt: new Date()
      },
      {
        point: 3,
        hero_id: 2,
        createdAt: new Date(2020, 9, 15),
        updatedAt: new Date()
      },
      {
        point: 5,
        hero_id: 2,
        createdAt: new Date(2020, 9, 16),
        updatedAt: new Date()
      },
      {
        point: 2,
        hero_id: 2,
        createdAt: new Date(2020, 9, 17),
        updatedAt: new Date()
      },
      {
        point: 6,
        hero_id: 2,
        createdAt: new Date(2020, 9, 18),
        updatedAt: new Date()
      },
      {
        point: 2,
        hero_id: 2,
        createdAt: new Date(2020, 9, 19),
        updatedAt: new Date()
      },
      {
        point: 6,
        hero_id: 2,
        createdAt: new Date(2020, 9, 20),
        updatedAt: new Date()
      },
      {
        point: 8,
        hero_id: 2,
        createdAt: new Date(2020, 9, 21),
        updatedAt: new Date()
      },
      {
        point: 2,
        hero_id: 2,
        createdAt: new Date(2020, 9, 22),
        updatedAt: new Date()
      },
      {
        point: 2,
        hero_id: 2,
        createdAt: new Date(2020, 9, 23),
        updatedAt: new Date()
      },
      {
        point: 4,
        hero_id: 2,
        createdAt: new Date(2020, 9, 24),
        updatedAt: new Date()
      },
      {
        point: 5,
        hero_id: 2,
        createdAt: new Date(2020, 9, 25),
        updatedAt: new Date()
      },
      {
        point: 5,
        hero_id: 2,
        createdAt: new Date(2020, 9, 26),
        updatedAt: new Date()
      },
      {
        point: 9,
        hero_id: 2,
        createdAt: new Date(2020, 9, 27),
        updatedAt: new Date()
      },
      {
        point: 8,
        hero_id: 2,
        createdAt: new Date(2020, 9, 28),
        updatedAt: new Date()
      },
      {
        point: 9,
        hero_id: 2,
        createdAt: new Date(2020, 9, 29),
        updatedAt: new Date()
      },
      {
        point: 4,
        hero_id: 2,
        createdAt: new Date(2020, 9, 30),
        updatedAt: new Date()
      },
      {
        point: 4,
        hero_id: 2,
        createdAt: new Date(2020, 10, 1),
        updatedAt: new Date()
      },
      {
        point: 3,
        hero_id: 2,
        createdAt: new Date(2020, 10, 2),
        updatedAt: new Date()
      },
      {
        point: 8,
        hero_id: 2,
        createdAt: new Date(2020, 10, 3),
        updatedAt: new Date()
      },
      {
        point: 2,
        hero_id: 2,
        createdAt: new Date(2020, 10, 4),
        updatedAt: new Date()
      },
      {
        point: 2,
        hero_id: 2,
        createdAt: new Date(2020, 10, 5),
        updatedAt: new Date()
      },
      {
        point: 6,
        hero_id: 2,
        createdAt: new Date(2020, 10, 6),
        updatedAt: new Date()
      },
      {
        point: 2,
        hero_id: 2,
        createdAt: new Date(2020, 10, 7),
        updatedAt: new Date()
      },
      {
        point: 3,
        hero_id: 2,
        createdAt: new Date(2020, 10, 8),
        updatedAt: new Date()
      },
      {
        point: 6,
        hero_id: 2,
        createdAt: new Date(2020, 10, 9),
        updatedAt: new Date()
      },
      {
        point: 9,
        hero_id: 2,
        createdAt: new Date(2020, 10, 10),
        updatedAt: new Date()
      },
      {
        point: 8,
        hero_id: 2,
        createdAt: new Date(2020, 10, 11),
        updatedAt: new Date()
      },
      {
        point: 0,
        hero_id: 2,
        createdAt: new Date(2020, 10, 12),
        updatedAt: new Date()
      },
      {
        point: 6,
        hero_id: 2,
        createdAt: new Date(2020, 10, 13),
        updatedAt: new Date()
      },
      {
        point: 5,
        hero_id: 2,
        createdAt: new Date(2020, 10, 14),
        updatedAt: new Date()
      },
      {
        point: 8,
        hero_id: 2,
        createdAt: new Date(2020, 10, 15),
        updatedAt: new Date()
      },
      {
        point: 5,
        hero_id: 2,
        createdAt: new Date(2020, 10, 16),
        updatedAt: new Date()
      },
      {
        point: 6,
        hero_id: 2,
        createdAt: new Date(2020, 10, 17),
        updatedAt: new Date()
      },
      {
        point: 3,
        hero_id: 2,
        createdAt: new Date(2020, 10, 18),
        updatedAt: new Date()
      },
      {
        point: 2,
        hero_id: 2,
        createdAt: new Date(2020, 10, 19),
        updatedAt: new Date()
      },
      {
        point: 6,
        hero_id: 2,
        createdAt: new Date(2020, 10, 20),
        updatedAt: new Date()
      },
      {
        point: 2,
        hero_id: 2,
        createdAt: new Date(2020, 10, 21),
        updatedAt: new Date()
      },
      {
        point: 9,
        hero_id: 2,
        createdAt: new Date(2020, 10, 22),
        updatedAt: new Date()
      },
      {
        point: 8,
        hero_id: 2,
        createdAt: new Date(2020, 10, 23),
        updatedAt: new Date()
      },
      {
        point: 6,
        hero_id: 2,
        createdAt: new Date(2020, 10, 24),
        updatedAt: new Date()
      },
      {
        point: 1,
        hero_id: 2,
        createdAt: new Date(2020, 10, 25),
        updatedAt: new Date()
      },
      {
        point: 10,
        hero_id: 2,
        createdAt: new Date(2020, 10, 26),
        updatedAt: new Date()
      },
      {
        point: 6,
        hero_id: 2,
        createdAt: new Date(2020, 10, 27),
        updatedAt: new Date()
      },
      {
        point: 10,
        hero_id: 2,
        createdAt: new Date(2020, 10, 28),
        updatedAt: new Date()
      },
      {
        point: 9,
        hero_id: 2,
        createdAt: new Date(2020, 10, 29),
        updatedAt: new Date()
      },
      {
        point: 5,
        hero_id: 2,
        createdAt: new Date(2020, 10, 30),
        updatedAt: new Date()
      },
      {
        point: 3,
        hero_id: 2,
        createdAt: new Date(2020, 11, 1),
        updatedAt: new Date()
      },
      {
        point: 5,
        hero_id: 2,
        createdAt: new Date(2020, 11, 2),
        updatedAt: new Date()
      },
      {
        point: 10,
        hero_id: 2,
        createdAt: new Date(2020, 11, 3),
        updatedAt: new Date()
      },
      {
        point: 1,
        hero_id: 2,
        createdAt: new Date(2020, 11, 4),
        updatedAt: new Date()
      },
      {
        point: 8,
        hero_id: 2,
        createdAt: new Date(2020, 11, 5),
        updatedAt: new Date()
      },
      {
        point: 2,
        hero_id: 2,
        createdAt: new Date(2020, 11, 6),
        updatedAt: new Date()
      },
      {
        point: 2,
        hero_id: 2,
        createdAt: new Date(2020, 11, 7),
        updatedAt: new Date()
      },
      {
        point: 8,
        hero_id: 2,
        createdAt: new Date(2020, 11, 8),
        updatedAt: new Date()
      },
      {
        point: 8,
        hero_id: 2,
        createdAt: new Date(2020, 11, 9),
        updatedAt: new Date()
      },
      {
        point: 6,
        hero_id: 2,
        createdAt: new Date(2020, 11, 10),
        updatedAt: new Date()
      },
      {
        point: 1,
        hero_id: 2,
        createdAt: new Date(2020, 11, 11),
        updatedAt: new Date()
      },
      {
        point: 3,
        hero_id: 2,
        createdAt: new Date(2020, 11, 12),
        updatedAt: new Date()
      },
      {
        point: 2,
        hero_id: 2,
        createdAt: new Date(2020, 11, 13),
        updatedAt: new Date()
      },
      {
        point: 9,
        hero_id: 2,
        createdAt: new Date(2020, 11, 14),
        updatedAt: new Date()
      },
      {
        point: 3,
        hero_id: 2,
        createdAt: new Date(2020, 11, 15),
        updatedAt: new Date()
      },
      {
        point: 5,
        hero_id: 2,
        createdAt: new Date(2020, 11, 16),
        updatedAt: new Date()
      },
      {
        point: 3,
        hero_id: 2,
        createdAt: new Date(2020, 11, 17),
        updatedAt: new Date()
      },
      {
        point: 7,
        hero_id: 2,
        createdAt: new Date(2020, 11, 18),
        updatedAt: new Date()
      },
      {
        point: 2,
        hero_id: 2,
        createdAt: new Date(2020, 11, 19),
        updatedAt: new Date()
      },
      {
        point: 6,
        hero_id: 2,
        createdAt: new Date(2020, 11, 20),
        updatedAt: new Date()
      },
      {
        point: 5,
        hero_id: 2,
        createdAt: new Date(2020, 11, 21),
        updatedAt: new Date()
      },
      {
        point: 3,
        hero_id: 2,
        createdAt: new Date(2020, 11, 22),
        updatedAt: new Date()
      },
      {
        point: 6,
        hero_id: 2,
        createdAt: new Date(2020, 11, 23),
        updatedAt: new Date()
      },
      {
        point: 3,
        hero_id: 2,
        createdAt: new Date(2020, 11, 24),
        updatedAt: new Date()
      },
      {
        point: 3,
        hero_id: 2,
        createdAt: new Date(2020, 11, 25),
        updatedAt: new Date()
      },
      {
        point: 3,
        hero_id: 2,
        createdAt: new Date(2020, 11, 26),
        updatedAt: new Date()
      },
      {
        point: 6,
        hero_id: 2,
        createdAt: new Date(2020, 11, 27),
        updatedAt: new Date()
      },
      {
        point: 9,
        hero_id: 2,
        createdAt: new Date(2020, 11, 28),
        updatedAt: new Date()
      },
      {
        point: 6,
        hero_id: 2,
        createdAt: new Date(2020, 11, 29),
        updatedAt: new Date()
      },
      {
        point: 9,
        hero_id: 2,
        createdAt: new Date(2020, 11, 30),
        updatedAt: new Date()
      },
      {
        point: 2,
        hero_id: 2,
        createdAt: new Date(2020, 11, 31),
        updatedAt: new Date()
      },
    ], {});
  },

  down: (queryInterface, Sequelize) => {
    return queryInterface.bulkDelete('ActivityLogs', null, {});
  }
};